...
interface TradesListProps {
  ...
  onReplicateTrade?: (trade: Trade) => void;
  isLoading?: boolean;
  maxHeight?: string;
}

export function TradesList({ title, trades, isFriendList, onReplicateTrade, isLoading, maxHeight = "400px" }: TradesListProps) {
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          {isFriendList ? "Trades executed by your friend." : "Trades replicated in your account."}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {isFriendList && <p className="text-xs text-muted-foreground mt-1">Ensure friend's API key is correct and they have recent activity.</p>}
          </div>
        ) : (
            ...
            <div className="space-y-4">
              {trades.map((trade) => (
                <TradeCard
                  ...
                  onReplicate={isFriendList ? onReplicateTrade : undefined}
                />
              ))}
            </div>
          </ScrollArea>
